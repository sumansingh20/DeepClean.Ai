...
export default function GitHubCallbackPage() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        const { token, user } = await handleOAuthCallback('github', code, state || undefined);
      ...
      <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl p-10 max-w-md w-full text-center">
        {status === 'loading' && (
          <>
            ...
            <h2 className="text-2xl font-black text-gray-900 mb-2">Authenticating with GitHub</h2>
